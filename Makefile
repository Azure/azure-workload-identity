...
PROXY_VERSION_VAR := $(REPO_PATH)/pkg/webhook.ProxySidecarVersion
LDFLAGS ?= "-X $(BUILD_TIME_VAR)=$(BUILD_TIMESTAMP) -X $(BUILD_VERSION_VAR)=$(IMAGE_VERSION) -X $(VCS_VAR)=$(BUILD_COMMIT) -X $(PROXY_VERSION_VAR)=$(IMAGE_VERSION)"
